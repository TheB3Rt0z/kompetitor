		...
		<table>
			<tr>
				...
				foreach ($distances as $key => $distance) {
					...
					<td class="a-right"><input type="text" name="riegel_calculator[forecasts][<?php echo $key ?>]" value="<?php echo $main->getPost('riegel_calculator', 'forecasts', $key) ?>" title="<?php echo trnslt('format: (h)h:mm:ss') ?>" readonly disabled /></td>
					<?php
					if (is_int((++$count) / 4))
						echo '</tr><tr>';
				}
