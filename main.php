        ...
        if (($_SESSION['status'] <= 1)
            ...
            if (isset($this->_post['bertoz_calculator']['distances'])) {
                ...
                foreach ($this->_post['bertoz_calculator']['distances'] as $key => $distance) {
                    ...
                    $forecast = round($calculator['time']->format('U') * pow($distance / $calculator['distance'], POW(BERTOZ_COEFFICIENT, 2 - BERTOZ_COEFFICIENT))) - 3600;
...
Main::addIdea('add a checkbox to stored pb and actual personals to include them or not in expected speed calculation');
    ...
    static function getVersion($base = 666) { // base should be set on first release
        ...
        if (function_exists('popen')) {
            ...
            $dir = popen('/usr/bin/du -sk -I .git '
                       . $_SERVER['DOCUMENT_ROOT'], 'r');//var_dump($dir, fgets($dir, 4096));
            
            if ($size = $status = fgets($dir, 4096)) {
            
                $size = substr($size, 0, strpos($size, "\t"));//var_dump($size, $base, $status);
    ...
    static function addLog($message, $type = 'debug') {
        ...
        switch ($code) {
            ...
            case self::LOG_ERROR: {
                $message = '<span class="log ' . $type . '"><pre>' . strtoupper($type) . ': ' . $message . '</pre></span>';
                break;
            }
            case self::LOG_INFO: {
